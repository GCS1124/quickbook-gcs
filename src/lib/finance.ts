...
export async function loadFinanceData(userId: string) {
  ...
  const [accounts, categories, transactions, budgets, goals, recurring, investments, loans, splits, notifications] = await Promise.all([
    ...
    supabase.from('finance_transactions').select('*, finance_accounts!finance_transactions_account_id_fkey(name), finance_categories(name, color, icon)').eq('user_id', userId).order('transaction_date', { ascending: false }).limit(2000),
