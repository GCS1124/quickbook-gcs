...
export default function App() {
  ...
  async function handleImportFiles(files: File[]) {
    ...
    try {
      ...
      if (demoMode) {
        const nextBatches: FinanceImportBatch[] = parsedFiles.map((parsed, index) => {
          const analysis = analyzeImportData(parsed);
          return { id: `demo-import-${Date.now()}-${index}`, user_id: 'demo', file_name: files[index].name, file_type: parsed.file_type, source_kind: parsed.source_kind, file_size: files[index].size, row_count: analysis.totalRows, imported_count: analysis.totalRows, duplicate_count: 0, total_amount: parsed.source_kind === 'payment_transactions' ? analysis.grossVolume : analysis.payoutTotal, total_fee: parsed.source_kind === 'payment_transactions' ? analysis.paymentFees : analysis.payoutFees, total_net: parsed.source_kind === 'payment_transactions' ? analysis.paymentNet : analysis.payoutTotal, currency_code: analysis.currency, status: 'completed', imported_at: new Date().toISOString() };
        });
        setImportedData((current) => ({ batches: [...nextBatches, ...current.batches], payments: [...parsedFiles.flatMap((item) => item.payments), ...current.payments], payouts: [...parsedFiles.flatMap((item) => item.payouts), ...current.payouts] }));
        setImportMessage(`${parsedFiles.reduce((sum, item) => sum + item.payments.length + item.payouts.length, 0)} records analysed in demo mode. Sign in to persist them to your workspace.`);
