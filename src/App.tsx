...
function ImportCenterView({ shopifyUserId, importedData, onImportFiles, onShopifyImport, onDeleteBatch, loading, error, message, shopifySyncLoading, shopifySyncError, shopifySyncMessage }: { shopifyUserId?: string; importedData: ImportData; onImportFiles: (files: File[]) => Promise<boolean>; onShopifyImport: (period: ShopifyImportPeriod, shopifyPageUrl?: string) => Promise<boolean>; onDeleteBatch: (batch: FinanceImportBatch) => void; loading: boolean; error: string; message: string; shopifySyncLoading: boolean; shopifySyncError: string; shopifySyncMessage: string }) {
  ...
  const shopifyConnectionTitle = shopifyMode === 'cli'
    ...
    : shopifyMode === 'client_credentials'
      ...
      : 'Shopify auto-link connection';
  const shopifyConnectionSubcopy = shopifyMode === 'cli'
    ? 'Auto-links the domain for this GCS Books user · no app selection or browser token'
    : shopifyMode === 'client_credentials'
      ? 'Runs on Vercel without browser approval · credentials and tokens stay server-side'
      : 'Links this GCS Books user to their store · approval once, token encrypted server-side';
  const shopifyConnectionHelp = shopifyMode === 'cli'
    ? 'Orders, product costs, payments, and payouts are synced for the selected period. Paste the actual store.myshopify.com domain and GCS Books automatically routes this import to that store for this signed-in user. Existing Shopify CLI authorization is reused and opens only when needed; no app selection is required. Shopify cannot expose every merchant cost, so add apps, ads, fulfilment, payroll, and other operating expenses with the template to complete the holistic P&L.'
    : shopifyMode === 'client_credentials'
      ...
      : 'Orders, product costs, payments, and payouts are synced for the selected period. The first import opens Shopify approval for this GCS Books user, then returns here and continues automatically. Use the canonical store.myshopify.com domain so the correct store is linked without guessing. Shopify cannot expose every merchant cost, so add apps, ads, fulfilment, payroll, and other operating expenses with the template to complete the holistic P&L.';
  const analysis = analyzeImportData(importedData);
  ...
  const syncShopify = async () => {
    ...
    if (!pageUrl) {
      setShopifyPageUrlError('Paste your actual Shopify store domain before starting the import.');
      return;
    }
