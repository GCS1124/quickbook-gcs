...
export default function App() {
  ...
  async function handleImportFiles(files: File[]) {
    ...
    try {
      ...
      if (demoMode) {
        const seenKeys = new Set([...importedData.payments.map((item) => item.source_key), ...importedData.payouts.map((item) => item.source_key)]);
        const freshParsedFiles = parsedFiles.map((parsed) => ({
          ...parsed,
          payments: parsed.payments.filter((item) => { if (seenKeys.has(item.source_key)) return false; seenKeys.add(item.source_key); return true; }),
          payouts: parsed.payouts.filter((item) => { if (seenKeys.has(item.source_key)) return false; seenKeys.add(item.source_key); return true; }),
        }));
        const nextBatches: FinanceImportBatch[] = freshParsedFiles.map((parsed, index) => {
          const analysis = analyzeImportData(parsed);
          const originalValidCount = parsedFiles[index].payments.length + parsedFiles[index].payouts.length;
          const originalCount = originalValidCount + parsed.reviewRows.length;
          return { id: `demo-import-${Date.now()}-${index}`, user_id: 'demo', file_name: files[index].name, file_type: parsed.file_type, source_kind: parsed.source_kind, file_size: files[index].size, row_count: originalCount, imported_count: analysis.totalRows, duplicate_count: originalValidCount - analysis.totalRows, review_count: parsed.reviewRows.length, total_amount: parsed.source_kind !== 'payouts' ? analysis.grossVolume : analysis.payoutTotal, total_fee: parsed.source_kind !== 'payouts' ? analysis.paymentFees : analysis.payoutFees, total_net: parsed.source_kind !== 'payouts' ? analysis.paymentNet : analysis.payoutTotal, currency_code: analysis.currency, status: 'completed', imported_at: new Date().toISOString() };
        });
        ...
        setImportedData((current) => ({ batches: [...nextBatches, ...current.batches], payments: [...importedPaymentRows, ...current.payments], payouts: [...importedPayoutRows, ...current.payouts] }));
        ...
        setImportMessage(`${totalImported} imported in demo mode${duplicateCount ? ` · ${duplicateCount} duplicates skipped` : ''}${reviewCount ? ` · ${reviewCount} need review` : ''}. Sign in to persist them to your workspace.`);
      } else if (supabase && currentUserId) {
        let importedPayments = 0; let importedPayouts = 0; let duplicates = 0; let reviewCount = 0;
        for (let index = 0; index < parsedFiles.length; index += 1) {
          ...
          importedPayments += result.importedPayments; importedPayouts += result.importedPayouts; duplicates += result.duplicateCount; reviewCount += result.reviewCount;
        }
        setImportedData(await loadImportData(currentUserId));
        await refreshData(currentUserId);
        setImportMessage(`${importedPayments + importedPayouts} imported${duplicates ? ` · ${duplicates} duplicates skipped` : ''}${reviewCount ? ` · ${reviewCount} need review` : ''}.`);
      } else {
  ...
  async function deleteImport() {
    ...
    try {
      if (demoMode) {
        ...
        await deleteImportedBatch(currentUserId, batch.id);
        await refreshData(currentUserId);
      }
...
function renderView(view: View, data: FinanceData, stats: { balance: number; income: number; expenses: number; savings: number; investmentValue: number; savingsRate: number }, categorySpend: { name: string; color: string; total: number }[], insights: { kind: string; title: string; body: string; icon: string }[], filteredTransactions: FinanceTransaction[], search: string, transactionFilter: 'all' | 'income' | 'expense' | 'transfer', setSearch: (value: string) => void, setTransactionFilter: (value: 'all' | 'income' | 'expense' | 'transfer') => void, setView: (value: View) => void, setModal: (value: Modal) => void, setTransactionForm: (value: TransactionForm) => void, markBillPaid: (bill: RecurringPayment) => void, setConfirmDelete: (value: string | null) => void, setConfirmImportDelete: (value: FinanceImportBatch | null) => void, showToast: (value: string) => void, exportCsv: () => void, importedData: ImportData, onImportFiles: (files: File[]) => void, importLoading: boolean, importError: string, importMessage: string) {
  switch (view) {
    ...
    case 'imports': return <ImportCenterView importedData={importedData} onImportFiles={onImportFiles} onDeleteBatch={setConfirmImportDelete} loading={importLoading} error={importError} message={importMessage} />;
...
function ConfirmDialog({ title, body, onCancel, onConfirm, confirmLabel = 'Delete', disabled = false }: { title: string; body: string; onCancel: () => void; onConfirm: () => void; confirmLabel?: string; disabled?: boolean }) { return <div className="modal-backdrop"><div className="confirm-dialog"><div className="confirm-icon">!</div><h2>{title}</h2><p>{body}</p><div className="modal-actions"><button className="secondary-button" disabled={disabled} onClick={onCancel}>Keep it</button><button className="danger-button" disabled={disabled} onClick={onConfirm}>{confirmLabel}</button></div></div></div>; }
