...
The import center now includes **Import from Shopify**. It asks for a store when one is not already configured or connected, plus the reporting period, then the Vite server runs Shopify CLI's authenticated Admin GraphQL queries for orders, product costs, Shopify Payments transactions, and payouts. The returned records are converted into the same source-aware import pipeline used by uploaded files, so they flow through the ledger, P&L, reports, analytics, and calendar.

This bridge is intentionally server-side: no Admin token is sent to the browser. During local development, the request is handled by the Vite middleware and the CLI authorizes the current GCS Books user on first use. The import dialog discovers locally authenticated CLI stores and also accepts a store entered at runtime; no `SHOPIFY_STORE_DOMAIN` variable is required:
