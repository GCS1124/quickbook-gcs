...
This bridge is intentionally server-side: no Admin token is sent to the browser. During local development, the request is handled by the Vite middleware and the CLI authorizes the selected Shopify store on first use. The import dialog discovers locally authenticated CLI stores and also accepts a Shopify admin page URL at runtime; no `SHOPIFY_STORE_DOMAIN` variable is required. URLs such as `https://admin.shopify.com/store/your-store/orders` and `https://your-store.myshopify.com/admin` are accepted:
