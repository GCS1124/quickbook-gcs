...
The first-use authorization requests `read_orders,read_all_orders,read_products,read_inventory,read_shopify_payments_accounts,read_shopify_payments_payouts`. Set `SHOPIFY_CLI_SCOPES` only if the store’s approved permissions need a deliberate override. The server serializes CLI work and always submits the resulting import under the authenticated GCS Books user, so user data stays isolated even when the local CLI session is shared by the machine. See the official [store auth](https://shopify.dev/docs/api/shopify-cli/store/store-auth) and [store execute](https://shopify.dev/docs/api/shopify-cli/store/store-execute) references.

### Production Shopify connection

Vercel cannot keep a developer machine’s interactive Shopify CLI session. The deployed app therefore uses the server-side OAuth bridge at `/api/shopify/import` for per-user stores: the signed-in user enters the actual canonical store domain, approves the Shopify app once, and GCS Books saves that connection only for that user. Local development continues to use Shopify CLI auto-link. A client-credentials mode is also available only for a fixed store owned by the same Shopify organization as the app. Both modes use the same server-only Admin GraphQL/import pipeline, keep each GCS Books user scoped through Supabase, and never send a Shopify token to browser storage.

For the per-user Vercel flow, keep `SHOPIFY_CONNECTION_MODE=oauth` (or omit it because OAuth is the default). The first import opens Shopify approval for the signed-in user’s supplied domain, then the callback returns to the import center and the import continues automatically. No `SHOPIFY_STORE_DOMAIN` or per-user environment variable is needed. The Shopify app itself must allow the production callback URL and use a distribution/installation model that permits the merchant stores you intend to connect.
