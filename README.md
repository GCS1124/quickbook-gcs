...
The import center now includes **Import from Shopify**. On first use for each signed-in GCS Books user, it asks for that user's Shopify admin page URL and reporting period, then the Vite server runs Shopify CLI's authenticated Admin GraphQL queries for orders, product costs, Shopify Payments transactions, and payouts. The returned records are converted into the same source-aware import pipeline used by uploaded files, so they flow through the ledger, P&L, reports, analytics, and calendar.

This bridge is intentionally server-side: no Admin token is sent to the browser. During local development, the request is handled by the Vite middleware and the CLI authorizes the selected Shopify store on first use. The import dialog discovers locally authenticated CLI stores and also accepts a Shopify admin page URL at runtime; no `SHOPIFY_STORE_DOMAIN` variable is required. The URL preference is kept in a browser key scoped to the signed-in GCS Books user, while production Shopify tokens are encrypted in a separate Supabase connection row per user. URLs such as `https://admin.shopify.com/store/your-store/orders` and `https://your-store.myshopify.com/admin` are accepted:

1. Copy `.env.example` to `.env.local`; leave `SHOPIFY_STORE_DOMAIN` unset if the signed-in user should choose a store in the dialog.
2. Start the app with `npm run dev`, sign in, open Shopify import center, click **Import from Shopify**, paste the Shopify page URL, and choose **Last month**, **Last 3 months**, **Last 6 months**, **Last 1 year**, or **Lifetime**. If this user/store pair is not authorized yet, the button starts `shopify store auth`, opens Shopify’s approval page, and continues the import after approval. There is no separate pre-auth command.
