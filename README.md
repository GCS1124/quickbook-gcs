...
This bridge is intentionally server-side: no Admin token is sent to the browser. During local development, the request is handled by the Vite middleware and the CLI authorizes the current GCS Books user on first use. The store can be entered in the import dialog at runtime; `SHOPIFY_STORE_DOMAIN` is only an optional default:

1. Copy `.env.example` to `.env.local`; optionally set `SHOPIFY_STORE_DOMAIN=your-store.myshopify.com` as the default for the local CLI bridge.
